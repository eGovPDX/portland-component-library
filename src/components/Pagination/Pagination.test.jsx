...
describe('Pagination', () => {
  ...
  describe('Navigation buttons', () => {
    test('renders previous and next buttons', () => {
      renderWithI18n(<Pagination {...defaultProps} currentPage={5} />);
    ...
    test('calls onPageChange when previous button is clicked', () => {
      ...
      renderWithI18n(<Pagination {...defaultProps} currentPage={5} onPageChange={onPageChange} />);
    ...
    test('does not call onPageChange for disabled buttons', () => {
      const onPageChange = jest.fn();
      renderWithI18n(<Pagination {...defaultProps} currentPage={1} onPageChange={onPageChange} />);
  ...
  describe('Page buttons', () => {
    test('renders correct page numbers for few pages', () => {
      renderWithI18n(<Pagination {...defaultProps} currentPage={2} totalPages={5} />);
      
      // Should show all pages 1-5
      for (let i = 1; i <= 5; i++) {
        // i18n aria-label is "Go to page X"; also match "Page X" for robustness
        const pattern = new RegExp(`(go to )?page ${i}`, 'i');
        expect(screen.getByRole('button', { name: pattern })).toBeInTheDocument();
      }
    });

    test('marks current page correctly', () => {
      renderWithI18n(<Pagination {...defaultProps} currentPage={3} totalPages={5} />);
    ...
    test('calls onPageChange when page button is clicked', () => {
      const onPageChange = jest.fn();
      renderWithI18n(<Pagination {...defaultProps} currentPage={1} totalPages={5} onPageChange={onPageChange} />);
    ...
    test('does not call onPageChange for current page button', () => {
      const onPageChange = jest.fn();
      renderWithI18n(<Pagination {...defaultProps} currentPage={3} totalPages={5} onPageChange={onPageChange} />);
  ...
  describe('Ellipsis', () => {
    test('shows ellipsis for many pages', () => {
      renderWithI18n(<Pagination {...defaultProps} currentPage={10} totalPages={20} />);
    ...
    test('shows correct page elements with ellipsis and first/last controls', () => {
      renderWithI18n(<Pagination {...defaultProps} currentPage={10} totalPages={24} />);
    ...
    test('hides ellipsis when showEllipsis is false (renders all page buttons)', () => {
      renderWithI18n(<Pagination {...defaultProps} currentPage={5} totalPages={20} showEllipsis={false} />);
      
      // Should show all pages 1-20
      for (let i = 1; i <= 20; i++) {
        const pagePattern = i === 5 
          ? /(current page).*page 5/i
          : new RegExp(`(go to )?page ${i}$`, 'i');
  ...
  test('handles single page correctly', () => {
    renderWithI18n(<Pagination {...defaultProps} currentPage={1} totalPages={1} />);
  ...
  describe('Custom props', () => {
    test('uses custom aria label', () => {
      renderWithI18n(<Pagination {...defaultProps} ariaLabel="Search results pagination" />);
    ...
    test('uses custom previous and next text', () => {
      renderWithI18n(<Pagination {...defaultProps} previousButtonText="Anterior" nextButtonText="Siguiente" />);
    ...
    test('applies custom className', () => {
      renderWithI18n(<Pagination {...defaultProps} className="custom-pagination" />);
    ...
    test('respects maxVisiblePages prop', () => {
      renderWithI18n(<Pagination {...defaultProps} currentPage={5} totalPages={20} maxVisiblePages={5} />);
      ...
      const pageButtons = screen.getAllByRole('button').filter(button => 
        ...
        !/previous|next/i.test(button.getAttribute('aria-label') || '')
      );
  ...
  describe('Accessibility', () => {
    test('has proper ARIA attributes', () => {
      renderWithI18n(<Pagination {...defaultProps} currentPage={5} totalPages={10} />);
  ...
  describe('Edge cases', () => {
    test('handles very large page numbers', () => {
      renderWithI18n(<Pagination {...defaultProps} currentPage={500} totalPages={1000} />);
    ...
    test('handles missing onPageChange gracefully', () => {
      const { container } = renderWithI18n(<Pagination currentPage={1} totalPages={5} />);
    ...
    test('handles boundary conditions correctly', () => {
      ...
      const { rerender } = renderWithI18n(<Pagination {...defaultProps} currentPage={1} totalPages={100} />);
      expect(screen.getByRole('button', { name: /previous page/i })).toBeDisabled();
