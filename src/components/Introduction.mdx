...
## PopoverWithCard Example

The `PopoverWithCard` component is designed for displaying rich, interactive content (such as Cards with buttons, lists, or links) in a popover anchored to a trigger element. It is fully accessible, using `role="dialog"`, focus management, and keyboard dismissal. Use this component instead of Tooltip for anything more complex than a simple text hint.
...
<PopoverWithCard
  cardProps={{
    ...
    text: 'This popover contains interactive content.',
...
**Accessibility:**
- Uses `role="dialog"` and `aria-labelledby` for screen readers
- Focus moves into the popover when opened and returns to the trigger when closed
- Dismissible with Escape key or by clicking outside
- Suitable for interactive content (buttons, links, forms, etc.) 